...
func TestParserQuery(t *testing.T) {
	...
	cases := []struct {
		...
		{input: "foo", err: "module path is not in an expected format"},
		{input: "foo/bar", err: "module path is not in an expected format"},
		{input: "foo.com/bar/baz", err: "module path is not in an expected format"},
		{input: "foo.com/bar/baz@", err: "module path is not in an expected format"},
		{input: "foo.com/bar/baz@bad-commit-hash", err: "module path is not in an expected format"},
		{
